...
- Every component with meaningful styling gets a co-located CSS Module: `Foo.tsx` + `Foo.module.css`, imported as `import styles from './Foo.module.css'`. No CSS-in-JS, no global class soup — `styles.xxx` composition only (see `Button.tsx` for the `[styles.a, condition && styles.b].filter(Boolean).join(' ')` idiom for conditional classes).
- **Tailwind is installed but reserved for the auth screens only.** Everything else stays on CSS Modules — do not convert existing components. Setup is Tailwind v4 via `@tailwindcss/vite`, entry at `src/styles/tailwind.css`. That file imports `theme.css` + `utilities.css` but deliberately **omits Preflight**, because Preflight is a global element reset that would override `globals.css` and restyle every existing component. Keep it that way. `cn()` lives at `src/lib/utils.ts` (clsx + tailwind-merge), the `@/*` → `src/*` alias is set in both `vite.config.ts` and `tsconfig.app.json`, and `components.json` configures the shadcn CLI (`npx shadcn@latest add ...` drops into `src/components/ui/`).
